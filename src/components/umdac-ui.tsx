...
export function MerchCard({
  ...
  const statusClasses = {
    ...
    Limited: 'bg-amber-100 text-amber-700 ring-amber-200',
    'Out of stock': 'bg-rose-100 text-rose-700 ring-rose-200',
  }

  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="mb-4 flex h-40 items-end rounded-xl bg-gradient-to-br from-slate-900 via-sky-700 to-cyan-500 p-4 text-white">
        <div className="flex w-full items-center justify-between">
          <span className="rounded-full bg-white/15 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.2em]">
            Merch
          </span>
          <span className={`rounded-full px-2.5 py-1 text-[10px] font-semibold ring-1 ring-inset ${statusClasses[status]}`}>
            {status}
          </span>
        </div>
      </div>
      <div className="flex items-start justify-between gap-4">
        ...
        <p className="text-lg font-bold text-sky-700">{price}</p>
      </div>
      <button
        type="button"
        className="mt-5 inline-flex w-full items-center justify-center rounded-full border border-slate-900 bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2"
      >
        {status === 'Out of stock' ? 'Notify me' : 'Reserve interest'}
      </button>
    </article>
  )
}
